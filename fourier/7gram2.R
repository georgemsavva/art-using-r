...
components2 <- cbind(-100:100,sapply(-100:100, function(j) gft(vector2g,j,scale=1)))
components2 <- components2[order(-abs(components2[,2])),]

components3 <- cbind(-100:100,sapply(-100:100, function(j) gft(vector3g,j,scale=1)))
...
hue2=1
p=0
for(i in c(seq(0,1,l=1500),rep(1,100))){
  rotfactor <-  exp(-8i*pi*((sin(i*pi-pi/2)+1)))
  rotfactor <- 1
p=p+1
...
lines(plotted*rotfactor, pch=19, cex=.4, col=rgb(.2,0,.2,1), lwd=12)
lines(plotted3*rotfactor, pch=19, cex=.4, col=rgb(.2,0,.2,1), lwd=12)
lines(plotted5*rotfactor, pch=19, cex=.4, col=rgb(.2,0,.2,1), lwd=12)
z=plotted2*rotfactor
segments(x0 = Re(z), y0 = Im(z),x1 = Re(c(z[-1],NA)), y1=Im(c(z[-1],NA)), pch=19, cex=.4, col=hsv(hue2,.7,1,1), lwd=6)
...
for(j in 0:(split-1)){
  pointsToPlot <- getlines(2*pi*((sin(i*pi-pi/2)+1))+(2*j*pi/6), components3)
